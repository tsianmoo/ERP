...
export default function SupplierCodeRulesPage() {
  ...
  const fetchVariables = async () => {
    try {
      const response = await fetch('/api/suppliers/code-rules/variables')
      const result = await response.json()
      if (result.data) {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingRule 
        ? `/api/suppliers/code-rules/${editingRule.id}`
        : '/api/suppliers/code-rules'
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/suppliers/code-rules/${id}`, { method: 'DELETE' })
      if (response.ok) {
        toast({ title: '删除成功' })
        fetchRules()
      } else {
          ...
          {rules.map(rule => (
            <div key={rule.id} className="border rounded-lg p-4 hover:bg-gray-50">
              ...
              <div className="mt-2 text-sm text-gray-500">
                <span>编码格式：<code className="px-2 py-0.5 bg-gray-100 rounded text-xs">{formatElementsPreview(rule.elements)}</code></span>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="max-w-3xl mx-auto">
              ...
              <div className="bg-white border rounded-lg flex flex-col" style={{ minHeight: '400px' }}>
                <div className="px-4 py-3 border-b flex-shrink-0">
                  <h3 className="font-medium">编码规则</h3>
                  <p className="text-xs text-gray-500 mt-0.5">配置供应商编码的自动生成规则</p>
                </div>
