...
// POST /api/products/generate-field-value - 生成商品字段值
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 获取编码规则
    const ruleResponse = await fetch(`${process.env.JAVA_BACKEND_URL || 'http://localhost:8080'}/api/products/code-rules/${code_rule_id}`);
...
function resolveVariable(
  ...
  // 从属性值中查找（支持 product_ 前缀）
  if (attributeValues) {
    const productVarName = variableName.startsWith('product_') 
      ? variableName.substring(8) 
      : variableName;
    
    if (attributeValues[productVarName] !== undefined) {
      return String(attributeValues[productVarName] || '');
    }
  ...
  try {
    ...
    try {
      ...
      const selectResult = await client.query(
        'SELECT current_sequence FROM product_code_sequences_new WHERE rule_id = $1 AND prefix_key = $2',
        [ruleId, prefixKey]
      );
      ...
      // 更新或插入序列号
      await client.query(
        `INSERT INTO product_code_sequences_new (rule_id, prefix_key, current_sequence, updated_at)
